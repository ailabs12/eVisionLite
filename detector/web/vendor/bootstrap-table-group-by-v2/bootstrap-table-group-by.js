...
!function ($) {
    ...
    BootstrapTable.prototype.initSort = function () {
        ...
        if ((this.options.groupBy) && (this.options.groupByField !== '')) {
            ...
            if ((this.options.sortName != this.options.groupByField)) {
                ...
                //});
		this.data.sort(function(a, b) {
                    if (a[that.options.groupByField] < b[that.options.groupByField]) return -1;
                    if (a[that.options.groupByField] > b[that.options.groupByField]) return 1;
                    if (that.options.sortOrder === 'asc') {
                        if (a[that.options.sortName] < b[that.options.sortName]) return -1;
                        if (a[that.options.sortName] > b[that.options.sortName]) return 1;
                        return 0;
                    }
                    if (that.options.sortOrder === 'desc') {
                        if (a[that.options.sortName] > b[that.options.sortName]) return -1;
                        if (a[that.options.sortName] < b[that.options.sortName]) return 1;
                        return 0;
                    }
    ...
    BootstrapTable.prototype.initBody = function () {
		...
		//that.$body.find('tr[data-parent-index='+item.id+']').toggleClass('d-none');
            });
            ...
            this.$container.off('click', '.groupBy')
                .on('click', '.groupBy', function() {
                    ...
                    that.$body.find('tr[data-parent-index='+$(this).closest('tr').data('group-index')+']').toggleClass('d-none');
                });
