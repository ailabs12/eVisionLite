...
module.exports = function(params) {
    ...
    var urlObj = prepareUrl(params.onvifUrl);

    var urlDevice = urlObj.protocol + '//' + urlObj.hostname + (urlObj.port ? ':' + urlObj.port : '');

    typeof faultylabs == "undefined" && (faultylabs = {});

    faultylabs.MD5 = function(h) {
        function r(e) {
            for (var a = [], b = 0; b < e.length; b++) a = a.concat(q(e[b]));
            return a
        }

        function s(e) {
            for (var a = [], b = 0; b < 8; b++) a.push(e & 255), e >>>= 8;
            return a
        }
        ...
        function q(a) {
            for (var b = [], h = 0; h < a.length; h++)
                if (a.charCodeAt(h) <= 127) b.push(a.charCodeAt(h));
                else
                    for (var j = encodeURIComponent(a.charAt(h)).substr(1).split("%"), l = 0; l < j.length; l++) b.push(parseInt(j[l], 16));
            return b
        }

        function t(a, b, h, j) {
            for (var l = "", i = 0, k = 0, c = 3; c >= 0; c--) k =
                arguments[c], i = k & 255, k >>>= 8, i <<= 8, i |= k & 255, k >>>= 8, i <<= 8, i |= k & 255, k >>>= 8, i <<= 8, i |= k, i = (i >>> 0).toString(16), i = "00000000".substr(0, 8 - i.length) + i, l += i;
            return l
        }

        function p(a) {
            for (var b = Array(a.length), h = 0; h < a.length; h++) b[h] = a[h];
            return b
        }
        ...
        var a = null,
            o = null;
        typeof h == "string" ? a = q(h) : h.constructor == Array ? h.length === 0 ? a = h : typeof h[0] == "string" ? a = r(h) : typeof h[0] == "number" ? a = h : o = typeof h[0] : typeof ArrayBuffer != "undefined" ? h instanceof ArrayBuffer ? a = p(new Uint8Array(h)) :
            h instanceof Uint8Array || h instanceof Int8Array ? a = p(h) : h instanceof Uint32Array || h instanceof Int32Array || h instanceof Uint16Array || h instanceof Int16Array || h instanceof Float32Array || h instanceof Float64Array ? a = p(new Uint8Array(h.buffer)) : o = typeof h : o = typeof h;
        ...
        return function() {
            ...
            var m = a.length % 64;
            if (m > 56) {
                for (var j = 0; j < 64 - m; j++) a.push(0);
                m = a.length % 64
            }
            for (j = 0; j < 56 - m; j++) a.push(0);
            a = a.concat(s(h * 8));
    ...
    function firstLogin() {
        return new Promise((resolve, reject) => {
            ...
            request.post({
                ...
                if (err) reject(err);
                // console.log('firstLogin:', JSON.parse(response.body));
    ...
    function secondLogin(data) {
        return new Promise((resolve, reject) => {
            var e = data.params.realm,
                c = data.params.realm,
                f = hex_md5(params.onvifUser + ":" + e + ":" + params.onvifPass),
                f = hex_md5(params.onvifUser + ":" + c + ":" + f);
            ...
            request.post({
                ...
                if (err) reject(err);
                // console.log('secondLogin:', JSON.parse(response.body));
                resolve(JSON.parse(response.body));
            });  
        });
    }
    ...
    function accessDoor(data) {
        return new Promise((resolve, reject) => {
            ...
            request.post({
                ...
                if (err) reject(err);
                // console.log('accessDoor response json:', JSON.parse(response.body));
                resolve(JSON.parse(response.body));
            });  
        });
    }
    ...
    function openDoor(data) {
        return new Promise((resolve, reject) => {
            ...
            request.post({
                ...
                if (err) reject(err);
                // console.log('openDoor:', JSON.parse(response.body));
                resolve(JSON.parse(response.body));
            });  
        });
    }
    ...
    function destroyAccess(data) {
        return new Promise((resolve, reject) => {
            ...
            request.post({
                ...
                if (err) reject(err);
                // console.log('destroyAccess:', JSON.parse(response.body));
                resolve(JSON.parse(response.body));
            });
        });
    }
